...
#import "ViewController2_1_iPad.h"
#import "Sound.h"
#import "BGM.h"
...
int vertical[NUM2];//横移動の判別（0,1,2,3の４つを設定する）
...
@interface ViewController2_1_iPad ()

@end

@implementation ViewController2_1_iPad
...
- (void)viewDidLoad
{
    ...
    for(int i=0;i<2;i++){
        ...
        life_name[i] = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH*0.22-(i*WIDTH*0.015), HEIGHT*0.001+(i*HEIGHT*0.04), WIDTH*0.15+(i*WIDTH*0.03), WIDTH*0.1)];
    }
    ...
    for(int i=0;i<3;i++){//ライフの数
        for(int j=0;j<2;j++){//自分のライフか相手のライフかの判断
            lifeV[i][j] = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH*0.38+(WIDTH*i*0.08), HEIGHT*0.01+(j*HEIGHT*0.045), WIDTH*0.08, WIDTH*0.08)];
            switch (j) {
                case 0://自分のライフ
    ...
    stopButton.frame =CGRectMake(10,10,HEIGHT*0.088,HEIGHT*0.088);//位置と大きさ
    ...
    for(int i=0;i<NUM3;i++){
        imageV3[i]= [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH/2, -100, 0, 0)];//メモリ割当
    ...
    chargeV = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH * 0.65, 0, WIDTH*0.17,  WIDTH*0.17 )];
    ...
    ballTotalV = [[UIImageView alloc]initWithFrame:CGRectMake( WIDTH * 0.85, HEIGHT*0.001, WIDTH*0.13,  WIDTH*0.16 )];
...
-(void)receiveData:(NSData *)data fromPeer:(NSString *)peer inSession: (GKSession *)session context:(void *)context{
    ...
    if([string hasPrefix:@"ballComing"]){
        ...
        if([string isEqualToString:@"ballComing_blue"]){//blue
            size2[num2] = arc4random_uniform(18)+60;//30~39
            colorJudge2[num2] =1;
            ...
            size2[num2] = arc4random_uniform(38)+80;//40~59
            colorJudge2[num2] =2;
            ...
            size2[num2] = arc4random_uniform(38)+120;//60~79
            colorJudge2[num2] =3;
            ...
            size2[num2] = arc4random_uniform(30)+160;//80~15
            colorJudge2[num2] =4;
    ...
    if(!isStop && !isCountDown){
        ...
        if(CGRectContainsPoint(CGRectMake(0, HEIGHT*0.92, WIDTH, HEIGHT),location)){//指定された場所でのみ生成
            ...
            if([self isInStorage]){
                ...
                ballY[num] = location.y - (size[num]/2)-100;//タッチしたところが見えるように-100
...
-(void)goldenBall{
    ...
    dy[num] = 0.7*iPadNum;//金ボールだけ遅め
    colorJudge[num] =5;
    size[num] = 200;
    ballX[num] = location.x - (size[num]/2);//矩形の左側をballXとする
    ballY[num] = location.y - (size[num]/2)-100;//タッチしたところが見えるように-50
...
-(void)bigger_or_smaller:(NSTimer*)timer{
    ...
    if(isNotMove ==true){
        ...
        ballY[num] = newLocation.y - (size[num]/2)-100;//タッチしたところが見えるように-50
    }
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if(touchField){
        ...
        if(CGRectContainsPoint(CGRectMake(0, HEIGHT*0.92, WIDTH, HEIGHT),newLocation)){//指定された場所でのみ生成可能
            ...
            ballX[num] = newLocation.x -(size[num]/2);
            ballY[num] = newLocation.y -(size[num]/2)-100;
...
-(void)moveCPU:(NSTimer*)timer{
    ...
    if(a% 3000 == 0 || a==1){
        ...
        NSArray * array =  [difficulty battleMode_iPad];
...
-(void)ballProduce:(int)b int:(float) alpha{
    ...
    if([self isInStorage2]){
        ...
        if(a%b == 0 || a ==1 || rivalBall){//ボールの生成頻度
            ...
            if(!rivalBall){
                size2[num2] = arc4random_uniform(130)+60;//赤を少し少なめ
                ballY2[num2] = HEIGHT*0.07;
            }else{
                ballY2[num2] = HEIGHT*0.06;
            ...
            if(size2[num2]< 80){
                ...
                dy2[num2] =1.0*iPadNum + alpha;
                dx2[num2] = 1.0*iPadNum + alpha|| -1.0 *iPadNum- alpha;
                ...
                if(!rivalBall){
                    ...
                    if(colorJudge2[num2] == 1){//通常のライバルボールのみ
                        dy2[num2] =1.2*iPadNum + alpha;
                        dx2[num2] = 1.2*iPadNum + alpha|| -1.2 *iPadNum- alpha;
                    }
                }
                ...
                dy2[num2] = 0.8 *iPadNum+ alpha;
                dx2[num2] = 0.8 *iPadNum+ alpha || -0.8*iPadNum - alpha;
                ...
                dy2[num2] = 0.7*iPadNum + alpha;
                dx2[num2] = 0.7*iPadNum +alpha  || -0.7*iPadNum - alpha;
                ...
                if(!rivalBall){
                    ...
                    if(colorJudge2[num2] == 3){//通常のライバルボールのみ
                        dy2[num2] =0.9*iPadNum + alpha;
                        dx2[num2] = 0.9*iPadNum + alpha|| -0.9 *iPadNum- alpha;
                    }
                }
                ...
                dy2[num2] = 0.5*iPadNum + alpha;
                dx2[num2] = 0.5*iPadNum + alpha || -0.5*iPadNum -alpha;
            ...
            if(rivalBall){//相手から送られてきたボールはアニメーションつきで生成される
                [self ballAppear_animation_rival:colorJudge2[num2] int:ballX2[num2] float:HEIGHT*0.075 float:(int)size2[num2]];
            }else{
...
-(void)ballAppear_animation_rival:(int) color int:(float)xPos float:(float) yPos float: (int)size2{
    ...
    UIImageView * ballFallsV = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH/2,-200, size2, size2)];
        ...
        for(int j=0;j<NUM3;j++){
            ...
            if(isFire[i]){//指が離されたボールに関して
                ...
                if (CGRectIntersectsRect(newImageV[i].frame, newImageV3[j].frame )){//ボールがぶつかったら
                    ...
                    if(colorJudge[i] == colorJudge3[j]){//同じボールだったら(金ボールでも消えない)
...
-(void)lifeDisapper_animation:(int) i int:(int) j{
    ...
    UIImageView * life_animationV = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH*0.36+(WIDTH*(i-1)*0.1), HEIGHT*0.01+(j*HEIGHT*0.045), WIDTH*0.08, WIDTH*0.08)];
