...
@interface Network_play ()

@end

@implementation Network_play
...
- (void)viewDidLoad
{
    ...
    currentMatch = delegate.myMatch; // 保持用のプロパティを使用して対戦を保持する
    
   // MatchCallback * match_callback = [[MatchCallback alloc]init];
    ...
    for(int i=0;i<3;i++){//ライフの数
        for(int j=0;j<2;j++){//自分のライフか相手のライフかの判断
            // lifeV[i][j] = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH*0.35+(WIDTH*i*0.08), HEIGHT*0.025+(j*HEIGHT*0.04), WIDTH*0.08, WIDTH*0.08)];
    ...
    //停止ボタン
    UIImage * image = [UIImage imageNamed:@"quit_network.png"];//画像の取得
    quitButton =[UIButton buttonWithType:UIButtonTypeRoundedRect];//ボタン作成
    quitButton.frame =CGRectMake(10,10,HEIGHT*0.088,HEIGHT*0.088);//位置と大きさ
    [quitButton setBackgroundImage:image forState:UIControlStateNormal];  // 画像をセットす
    [quitButton addTarget:self action:@selector(cutButton) forControlEvents:UIControlEventTouchUpInside];
    ...
    itemBall_white = [UIImage imageNamed:@"itemBall_white.png"];
...
-(void)sendData{
    ...
    if(isCut){
        ...
        [self sendDataToAllPlayers:cutData];
    }
...
-(void)sendData_life:(int)lifeCount{
    ...
    [self sendDataToAllPlayers:lifeData];
...
-(void)sendData_ball:(int)ball_id{
    ...
    [self sendDataToAllPlayers:ballData];
...
-(void)sendData_ballTotal:(int)num{
    ...
    [self sendDataToAllPlayers:ballTotalData];
}
...
- (void)sendDataToAllPlayers:(NSData *)data{
    ...
    NSLog(@"send!!");
    [currentMatch sendDataToAllPlayers:data withDataMode:GKMatchSendDataUnreliable error:nil];
}
...
- (void)match:(GKMatch *)match didReceiveData:(NSData *)data fromPlayer:(NSString *)playerID{
    ...
    if([string hasPrefix:@"ballComing"]){
        ...
        imageV2[num2].frame= CGRectMake(WIDTH/2, -200, 0,0 );//メモリ割当
        [self.view addSubview:imageV2[num2]];//上から降ってくるイメージ
    }
...
-(void)lifeChange:(int) lifeCount{
    ...
    switch (lifeCount) {
        ...
        case 2:
            ...
            lifeV[2][1].image  = life_no_red;
            break;
        case 1:
            lifeV[0][1].image  = life_yes_red;
            for(int i=1;i<3;i++){
                lifeV[i][1].image  = life_no_red;
            }
            break;
        case 0:
            for(int i=0;i<3;i++){
...
-(void)ItemBall_produce{
    ...
    if([self isInStorage3]){
        ...
        int rand = arc4random_uniform(4);//0~3
