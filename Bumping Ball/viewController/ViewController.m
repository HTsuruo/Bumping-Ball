...

{
    ...
    //[ud removeObjectForKey:@"highScore_normal"];  // KEY_Iを削除する
   ...
   // isReverse = false;
    ...
    //scoreを表示するビュー
    scoreV = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH * 0.2, 15, WIDTH*0.3, HEIGHT*0.045)];
    UIImage * scoreImage = [UIImage imageNamed:@"scoreImage.png"];
    scoreV.image = scoreImage;
    [self.view addSubview:scoreV];
    
    //highscoreを表示するビュー
    HighScoreV = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH*0.675, 15, WIDTH*0.2, HEIGHT*0.05)];
    ...
    scoreView[0].frame = CGRectMake(WIDTH * 0.30, HEIGHT*0.058, WIDTH*0.08, HEIGHT*0.06);
    ...
    chargeV = [[UIImageView alloc]initWithFrame:CGRectMake(WIDTH * 0.49, HEIGHT*0.035, WIDTH*0.13,  WIDTH*0.13 )];
    ...
    //イベントが起きた時
    /*reverseV = [[UIImageView alloc]init];
    UIImage * reverse = [UIImage imageNamed:@"reverse.png"];
    reverseV.image = reverse;*/
...
-(void)stopMenu{
    ...
    if([NSLocalizedString(@"localizationKey", nil) isEqualToString:@"日本語"]){//日本語の場合
        ...
        exit = [UIImage imageNamed:@"exit.png"];//画像の取得
    } else {
        restart = [UIImage imageNamed:@"restart_eng.png"];//画像の取得
        exit = [UIImage imageNamed:@"exit_eng.png"];//画像の取得
    }
...
-(void)moveCPU:(NSTimer*)timer{
    ...
    [self difficulty];//ボールを生成(3.5inchと4inchの判別はメソッド内で行っている)
    
    for(int i=0;i<NUM2;i++){
        
        
        if(ballY2[i]+size2[i]<HEIGHT*0.95){//保管庫のy座標と相談が必要(storageY2)
            ...
            vc1_2.modalTransitionStyle = UIModalTransitionStyleCrossDissolve;
...
-(void)difficulty{
    ...
    if(HEIGHT == 460){//3.5inch の時
        
        //生成頻度と加速度の２つのパラメータをarrayに格納します
        array  = [difficulty dif_3_5];
        
    }else{//4inchの時
        
        //生成頻度と加速度の２つのパラメータをarrayに格納します
        array  = [difficulty dif_4];
...
-(void)ballProduce:(int)b float:(float)alpha{
    ...
    if([self isInStorage2]){
        ...
        if(a%b == 0 || a ==1){//ボールの生成頻度
            ...
            if([mode isEqualToString:@"easy"]){
                size2[num2] = arc4random_uniform(29)+30;
            }else{
                size2[num2] = arc4random_uniform(68)+30;
            }
            ...
            if(ballX2[num2]+size2[num2]>WIDTH){
                ballX2[num2] = ballX2[num2]-size2[num2];
            }
            ...
            if([mode isEqualToString:@"hard"]){
                [self ballColor_hard:(float)alpha];
            }else{
                [self ballColor:(float)alpha];
            }
            ...
            [self.view addSubview:imageV[num2]];
        }
        ...
        for(int j=0;j<NUM2;j++){
            ...
            if(isFire[i]){//指が離されたボールに関して
                ...
                if (CGRectIntersectsRect(newImageV[i].frame, newImageV2[j].frame )){//ボールがぶつかったら
                    ...
                    // NSLog(@"newImageV[%d]:%@",i,NSStringFromCGRect(newImageV[i].frame));
                    //NSLog(@"newImageV2[%d]:%@",j,NSStringFromCGRect(newImageV2[j].frame));
...
-(void)scoreCount{
    ...
    if(totalScore <1000){//3桁（2桁はあり得ない）
        ...
        for(int i=0;i<3;i++){
            scoreView[i].frame =CGRectMake(WIDTH * 0.37 -(i*between), HEIGHT*0.058, WIDTH*0.08, HEIGHT*0.06);
        }
        limit = 3;
    }else if (totalScore <10000){//4桁
        for(int i=0;i<4;i++){
            scoreView[i].frame =CGRectMake(WIDTH * 0.4 -(i*between), HEIGHT*0.058, WIDTH*0.08, HEIGHT*0.06);
        }
        limit =4;
    }else if (totalScore <100000){//5桁
        for(int i=0;i<5;i++){
            scoreView[i].frame =CGRectMake(WIDTH * 0.42 -(i*between), HEIGHT*0.058, WIDTH*0.08, HEIGHT*0.06);
        }
        limit =5;
    }else{//6桁
        for(int i=0;i<6;i++){
            scoreView[i].frame =CGRectMake(WIDTH * 0.43 -(i*between), HEIGHT*0.058, WIDTH*0.08, HEIGHT*0.06);
        }
        limit =6;
    }
...
-(void)scoreCount2{
    ...
    //最初の時は0を表示
    if(temp == 0){
        scoreView2[0].frame = CGRectMake(WIDTH*0.73, HEIGHT*0.058,WIDTH*0.08, HEIGHT*0.06);
        limit = 0;
        ...
        for(int i=0;i<4;i++){
            scoreView2[i].frame =CGRectMake(WIDTH*0.81-(between*i), HEIGHT*0.058, WIDTH*0.08, HEIGHT*0.06);
        }
        limit =4;
